(function (factory) {
    ...
    var ManageLogEntry = React.createClass({
        ...
        componentDidMount: function () {
            ...
            var textArea = this.refs.details;
            autoGrow(textArea);

            /**
             * Set focus to control
             */
            $(this.refs.performedat).focus();
        },
        ...
        handleChange: function (event) {
            if (event.target === this.refs.performedat) {
            ...
            else if (event.target === this.refs.kind) {
                this.setState({
                    kind: event.target.value
                });
            }
            else if (event.target === this.refs.actualduration) {
                var duration = 0;
                var durationParseResult = babble.get('durations').translate(this.refs.actualduration.value.trim());
            ...
            else if (event.target === this.refs.details) {
        ...
        handleSaveClick: function () {
            ...
            var logEntry = doozy.extrude(doozy.logEntry(), Object.assign({
                ...
                actionName: !this.state.actionId ? this.refs.action.value : null
            }, this.state));
