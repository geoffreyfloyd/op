 (function (factory) {
    ...
    function importActions (table) {
        table.forEach(function (row) {
            var gnode = new db.Gnode(db, row.Name, 'doozy.action', camelize(row));
    ...
    function importLogEntries (table) {
        table.forEach(function (row) {
            ...
            var gnode = new db.Gnode(db, name, 'doozy.logentry', camelize(row));
            gnode.born = gnode.state.created || gnode.state.enlist || gnode.born;
            db.add(gnode);
        });
    ...
    function importFocuses (table) {
        table.forEach(function (row) {
            var gnode = new db.Gnode(db, row.Name, 'doozy.focus', camelize(row));
            gnode.born = gnode.state.created || gnode.state.enlist || gnode.born;
            db.add(gnode);
        });
    ...
    function importPlans (table) {
        table.forEach(function (row) {
            var gnode = new db.Gnode(db, row.Name, 'doozy.plan', camelize(row));
            gnode.born = gnode.state.created || gnode.state.enlist || gnode.born;
            db.add(gnode);
        });
    ...
    function importPlanSteps (table) {
        var gnodes = [];
        table.forEach(function (row) {
            var gnode = new db.Gnode(db, row.Name, 'doozy.planstep', camelize(row));
            gnode.born = gnode.state.created || gnode.state.enlist || gnode.born;
            db.add(gnode);
    ...
    function importTargets (table) {
        table.forEach(function (row) {
            var gnode = new db.Gnode(db, row.Name, 'doozy.target', camelize(row));
            gnode.born = gnode.state.created || gnode.state.enlist || gnode.born;
            db.add(gnode);
        });
    ...
    //     table.forEach(function (row) {
    //         var name = row.Date;
            // var gnode = new db.Gnode(db, row.Name, 'doozy.logentry', camelize(row));
            // gnode.born = gnode.state.created || gnode.state.enlist || gnode.born;
            // db.add(gnode);
    //     });
